...
//--------------------------------------------
    ...
    setInterval(function(){
        ...
        if(bookChanTimeout>defaut_timeout){
            teminateApplication('Book Channel Timeout');
        }
...
//--------------------------------------------
    ...
    console.log(" ");
    utils.log(":::: :::: :::: :::: :::: :::: ::::");
    utils.log(":::: MONITOR BOT  HAS STARTED ::::");
    utils.log(":::: :::: :::: :::: :::: :::: ::::");
    console.log(" ");
    ...
    wss_auth.onmessage = (msg) => {
        ...
        if(messages.event == "auth"){
            ...
            else if(messages.channel == "book") CHAN_ID_books = messages.chanId;
    ...
    books_channel_listener = (data) => { 
        ...
        if(Array.isArray(data[0])){
            ...
            for(i=0;i<(data.length/2);i++){
                bids.push(data[i]);
                asks.push(data[i+(data.length/2)]);
                utils.log(('').padEnd(15)+(''+bids[i][0]).padEnd(15)+(''+bids[i][1]).padEnd(3)+(''+bids[i][2]).padEnd(20)+(''+asks[i][0]).padEnd(15)+(''+asks[i][1]).padEnd(3)+asks[i][2]);
            }
        }else{
            ...
            */
            ...
            if(data[1] > 0){
                ...
                //ADD OR UPDATE
                if(data[2]>0){
                    bids.forEach(function(res,i){
                        if(res[0]==data[0]){
                            //new bid price is already on the bid price list (update)
                            res[2] =+ data[2];
                        }else{
                            if(data[0] > bids[0][0]){
                                //new bid price is < than the first bid price in the list (add at first position)
                                bids.splice(0,0,data);
                            }else if((i == (bids.length-1)) && data[0] < bids[i][0]){
                                //new bid price is < than the last bid price in the list (add at the end)
                                bids.push(data);
                            }else if((data[0] < bids[i][0]) && (data[0] > bids[i+1][0])){
                                //new bid price is in the middle of the list (add in the middle)
                                bids.splice((i+1),0,data);
                            }
                        }
                    });
                }else{
                    asks.forEach(function(res,i){
                        if(res[0]==data[0]){
                            //new ask price is already on the ask price list (update)
                            res[2] =+ data[2];
                        }else{
                            if(data[0] < asks[0][0]){
                                //new ask price is < than the first ask price in the list (add at first position)
                                asks.splice(0,0,data);
                            }else if((i == (asks.length-1)) && data[0] > asks[i][0]){
                                //new ask price is < than the last ask price in the list (add at the end)
                                asks.push(data);
                            }else if((data[0] > asks[i][0]) && (data[0] < asks[i+1][0])){
                                //new ask price is in the middle of the list (add in the middle)
                                asks.splice((i+1),0,data);
                            }
                        }
                    });                    
                }
            ...
            let higher_ask_amount = 0;
            let higher_bid_amount = 0;

            let higher_ask_amount_orders = 0;
            let higher_bid_amount_orders = 0;

            let higher_ask_amount_at = 0;
            let higher_bid_amount_at = 0;

            let higher_ask_count = 0;
            let higher_bid_count = 0;

            let higher_ask_orders_amount = 0;
            let higher_bid_orders_amount = 0;

            let higher_ask_orders_at = 0;
            let higher_bid_orders_at = 0;

            console.log('\033c');

            console.log(('').padEnd(15)+'SPREAD: '+(asks[0][0]/bids[0][0])/100);

            console.log('');
            console.log('');

            console.log(('').padEnd(15)+('BIDS/BUYS').padEnd(38)+'ASKS/SELLS');
            for(i=0;i<len;i++){
                if(i<30){
                    console.log(('').padEnd(15)+(''+bids[i][1]).padEnd(3)+(''+bids[i][2]).padEnd(18)+(''+bids[i][0]).padEnd(18)+(''+asks[i][0]).padEnd(10)+(''+asks[i][2]).padEnd(18)+asks[i][1]);
                }

                if(bids[i][2] > higher_bid_amount){ higher_bid_amount = bids[i][2]; higher_bid_amount_at = bids[i][0]; higher_bid_amount_orders = bids[i][1]; }
                if(asks[i][2] < higher_ask_amount){ higher_ask_amount = asks[i][2]; higher_ask_amount_at = asks[i][0]; higher_ask_amount_orders = asks[i][1]; }

                if(bids[i][1] > higher_bid_count){ higher_bid_count = bids[i][1]; higher_bid_orders_at = bids[i][0]; higher_bid_orders_amount = bids[i][2]; }
                if(asks[i][1] > higher_ask_count){ higher_ask_count = asks[i][1]; higher_ask_orders_at = asks[i][0]; higher_ask_orders_amount = asks[i][2]; }
            ...
            console.log(('').padEnd(15)+'HIGHER ORDERS AND VOLUMES');
            console.log('');
            console.log(
                ('').padEnd(15)
                +('PRICE').padEnd(10)
                +('ORDERS').padEnd(8)
                +(''+('VOLUME').padStart(12)).padEnd(20)
                +('PRICE').padEnd(10)
                +('ORDERS').padEnd(8)
                +(''+('VOLUME').padStart(13)).padEnd(20)
                +'SPREAD'
            );

            console.log(
                ('').padEnd(15)
                +(''+higher_bid_amount_at).padEnd(10)
                +(''+higher_bid_amount_orders).padEnd(8)
                +('$'+(''+formatMoney((higher_bid_amount_at*higher_bid_amount),2,',','.')).padStart(11)).padEnd(20)
                +(''+higher_ask_amount_at).padEnd(10)
                +(''+higher_ask_amount_orders).padEnd(8)
                +('$ '+(''+formatMoney(Math.abs(higher_ask_amount_at*higher_ask_amount),2,',','.')).padStart(11)).padEnd(20)
                +(''+(higher_ask_amount_at/higher_bid_amount_at)).padEnd(20)
            );

            console.log(
                ('').padEnd(15)
                +(''+higher_bid_orders_at).padEnd(10)
                +(''+higher_bid_count).padEnd(8)
                +('$ '+(''+formatMoney((higher_bid_orders_at*higher_bid_orders_amount),2,',','.')).padStart(10)).padEnd(20)
                +(''+higher_ask_orders_at).padEnd(10)
                +(''+higher_ask_count).padEnd(8)
                +('$ '+(''+formatMoney(Math.abs(higher_ask_orders_at*higher_ask_orders_amount),2,',','.')).padStart(11)).padEnd(20)
                +(''+(higher_bid_orders_at/higher_ask_orders_at)).padEnd(20)
            );
